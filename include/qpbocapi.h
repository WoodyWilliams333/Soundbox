...
//设置闪卡记录超时时间
int qpboc_ext_set_mute_log_timeout(uint32_t uiTimeoutSec);
//设置闪卡最多笔数
int qpboc_ext_set_mute_log_max(uint32_t uiMaxLogCount);
...
void uicc_qpboc_online_oda_control(int enabled);
...
int uicc_qpboc_get_mute_record(int data_index, 
										unsigned char * pbuff, 
										uint32_t  * puiSize);
void uicc_qpboc_set_delete_log_flag(char is_compare_Un);

char uicc_qpboc_get_delete_log_flag(void);


//设置闪卡记录超时时间
int uicc_qpboc_mute_log_timeout_set(uint32_t uiTimeoutSec);
//设置闪卡最多笔数
int uicc_qpboc_mute_log_max_set(uint32_t uiMaxLogCount);
...
enum{
	ODA_ONLINE_DDA_APPROVE = 0,
	ODA_ONLINE_DDA_DECLIED,
	ODA_ONLINE_SDA_APPROVE,
	ODA_ONLINE_SDA_DECLIED,
	ODA_ONLINE_FAILED_DECLIED,
	ODA_ONLINE_NA = 0xff,
};
