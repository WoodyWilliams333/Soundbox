...
void sha256(const unsigned char *data, size_t len, unsigned char *out)
{
	...
	uint32_t w[64];
	size_t chunk_len = new_len / 64; //�?512bit区块
	uint32_t val = 0;
    ...
    bzero(buf + len, append); //将内存（字符串）前n个字节清�?<string.h>
    if (len > 0) {
        memcpy(buf, data, len);
    }
    ...
    for (int idx = 0; idx < chunk_len; idx++) {
        val = 0;
        for (int i = 0; i < 64; i++) {//将块分解�?16�?32-bit的big-endian的字，记为w[0], �?, w[15]
        ...
        for (int i = 16; i < 64; i++) {//�?16个字直接由以上消息的第i个块分解得到，其余的字由如下迭代公式得到�?
