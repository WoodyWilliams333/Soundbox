...
typedef int (*fn_emv_cl_kernel_param_get)(int data_index, 
										  void * pBuff,
										  uint32_t * BuffSize);

typedef int (*fn_emv_cl_apdu_send)(int fd, uint32_t sendlen, const void * senddata);
typedef int (*fn_emv_cl_apdu_resp_get)(int fd, int AllowStop, uint32_t *recvlen, void *recvdata, EMV_CL_APDU_STATUS * status);
...
int emv_cl_kernel_param_get(int data_index,
...
int emv_cl_entry_point( int ifd, int select_next, 
	                     ...
	                     unsigned char * aid, uint32_t * aid_len, 
	                     unsigned char * FCI, uint32_t * FCI_Len
	                  );
