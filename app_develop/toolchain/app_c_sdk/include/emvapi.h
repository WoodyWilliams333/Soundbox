...
#if 0
void nfc_dbg_gpio_cvt(void);
#define NFC_DEBUG_GPIO() nfc_dbg_gpio_cvt()
#else
#define NFC_DEBUG_GPIO()
#endif
...
typedef int (*fn_callback_apdu_exchange)(int fd, uint32_t sendlen, const void *senddata, uint32_t *recvlen, void *recvdata);
...
typedef int  (*fn_callback_emv_terminal_aid_get)(int data_index, void * pAidInfo, uint32_t *puiSize);
...
int emv_get_one_aid_info(int data_index, struct terminal_aid_info *info);
...
typedef int  (*fn_callback_emv_capk_get)(int data_index, struct issuer_ca_public_key *key);
...
int emv_get_one_ca_public_key(const void *RID, uint8_t data_index, struct issuer_ca_public_key *key);
int emv_get_one_ca_public_key_by_no(int no, struct issuer_ca_public_key *key);
int emv_delete_one_ca_public_key(const void *RID, uint8_t data_index);
...
#define KERNEL_TYPE_EMV_JCB     0x00000007
...
int emv_set_unknow_tags(const uint8_t * tag_list, uint32_t tag_list_len);
...
int emv_get_log_element(int data_index, void * tag, uint8_t * piTag, void *value, uint8_t *valuelen);
...
int emv_get_log_element_ec(int data_index, void * tag, uint8_t * piTag, void *value, uint8_t *valuelen);
...
//void emv_uicc_enable_oda_online_auth(int enable);
//int  emv_uicc_get_oda_online_transaction_status(void);
