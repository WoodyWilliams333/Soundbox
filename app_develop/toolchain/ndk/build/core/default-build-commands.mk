...
$(PRIVATE_CXX) \
    ...
    -shared \
    $(if $(PRIVATE_DISABLE_SYSROOT_FLAGS),,--sysroot=$(call host-path,$(PRIVATE_SYSROOT_LINK))) \
...
define cmd-build-executable-binary
$(TARGET_OBJCOPY) \
    --gap-fill=0xff -O binary \
    $(call host-path,$(LOCAL_BUILT_MODULE)) \
    $(call host-path,$(LOCAL_BUILT_MODULE)).bin
endef
...
ifeq ($(TARGET_NO_STDLIB),)
TARGET_LDLIBS:=-lc $(TARGET_LDLIBS)
else
TARGET_NO_STDLIB:=
endif
...
TARGET_CPP		= $(TOOLCHAIN_PREFIX)cpp
TARGET_CPPFLAGS :=
